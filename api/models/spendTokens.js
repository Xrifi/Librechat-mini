...
const { updateUsageRollup } = require('./quotaMethods');
...
const spendTokens = async (txData, tokenUsage) => {
  ...
  try {
    ...
    if (prompt || completion) {
      ...
      const totalValue = (prompt?.prompt ?? 0) + (completion?.completion ?? 0);
      if (totalValue !== 0) {
        await updateUsageRollup({ user: txData.user, amount: Math.abs(totalValue) });
      }
    } else {
