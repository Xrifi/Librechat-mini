...
describe('User Methods - Database Tests', () => {
  ...
  describe('createUser', () => {
    ...
    test('should assign admin role to the first created user when role is not provided', async () => {
      const firstUser = await methods.createUser(
        {
          name: 'First User',
      ...
      expect(firstUser).toHaveProperty('role', SystemRoles.ADMIN);

      const secondUser = await methods.createUser(
        {
          name: 'Second User',
      ...
      expect(secondUser).toHaveProperty('role', SystemRoles.USER);
    });
